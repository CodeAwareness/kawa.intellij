...
public class DiffViewerManager {
    ...
    public void showDiff(String localFilePath, String peerName, String peerContent) {
        ApplicationManager.getApplication().invokeLater(() -> {
            try {
                ...
                // Create diff request
                String title = "Kawa Code: " + fileName + " - " + peerName;
    ...
    /**
     * Open a diff viewer comparing two files that are already on disk.
     * Used for branch comparison where Gardener provides both file paths.
     *
     * @param userFilePath Path to the user's version of the file
     * @param peerFilePath Path to the peer/branch version of the file
     * @param title Title for the diff viewer
     */
    public void showDiffFiles(String userFilePath, String peerFilePath, String title) {
        ApplicationManager.getApplication().invokeLater(() -> {
            VirtualFile userFile = LocalFileSystem.getInstance().refreshAndFindFileByPath(userFilePath);
            if (userFile == null || !userFile.exists()) {
                Logger.warn("User file not found: " + userFilePath);
                return;
            }

            VirtualFile peerFile = LocalFileSystem.getInstance().refreshAndFindFileByPath(peerFilePath);
            if (peerFile == null || !peerFile.exists()) {
                Logger.warn("Peer file not found: " + peerFilePath);
                return;
            }

            DiffContentFactory contentFactory = DiffContentFactory.getInstance();
            DiffContent userContent = contentFactory.create(project, userFile);
            DiffContent peerContent = contentFactory.create(project, peerFile);

            String diffTitle = title != null ? title : "Kawa Code: Branch Comparison";
            SimpleDiffRequest request = new SimpleDiffRequest(
                diffTitle,
                userContent,
                peerContent,
            ...
            DiffManager.getInstance().showDiff(project, request);
            Logger.info("Opened diff viewer: " + diffTitle);
        });
    }
