...
/**
 * Status bar widget factory and implementation for Kawa Code.
...
public class CodeAwarenessStatusBarWidget implements StatusBarWidgetFactory {
    ...
    private static class CodeAwarenessWidget implements StatusBarWidget, StatusBarWidget.TextPresentation {
        ...
        public @NotNull String getText() {
            ...
            return settings.isHighlightsEnabled() ? "Kawa Code: ON" : "Kawa Code: OFF";
        }
        ...
        public @NotNull String getTooltipText() {
            ...
            return settings.isHighlightsEnabled()
                ? "Click to turn OFF Kawa Code highlights"
                : "Click to turn ON Kawa Code highlights";
        }
        ...
        public @Nullable Consumer<MouseEvent> getClickConsumer() {
            return (MouseEvent event) -> {
                ...
                Logger.info("Kawa Code toggled to: " + (newState ? "ON" : "OFF"));
                ...
                if (appService != null) {
                    if (newState) {
                        ...
                        if (!appService.isConnected()) {
                            Logger.info("Highlights enabled - connecting to Kawa Code backend asynchronously...");
                            // Run connection on background thread to avoid blocking EDT and accessing APIs too early
                            ApplicationManager.getApplication().executeOnPooledThread(() -> {
                                try {
                                    appService.connect();
                                    Logger.info("Successfully connected to Kawa Code backend");
                                    ...
                                    Logger.error("Failed to connect to Kawa Code backend when enabling highlights", e);
                            ...
                            Logger.info("Already connected to Kawa Code backend");
                        }
                    } else {
                    ...
                    Logger.warn("Kawa Code Application Service not available");
                }
            };
        }
