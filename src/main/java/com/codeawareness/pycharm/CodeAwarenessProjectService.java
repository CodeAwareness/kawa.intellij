...
import com.codeawareness.pycharm.events.handlers.AuthLoginHandler;
import com.codeawareness.pycharm.events.handlers.AuthLogoutHandler;
import com.codeawareness.pycharm.events.handlers.BranchSelectHandler;
import com.codeawareness.pycharm.events.handlers.BranchUnselectHandler;
...
import com.codeawareness.pycharm.events.handlers.SyncRefreshHandler;
...
/**
 * Project-level service for Kawa Code plugin.
...
public final class CodeAwarenessProjectService implements Disposable, SettingsChangeListener {
    ...
    public CodeAwarenessProjectService(Project project) {
        ...
        Logger.info("Kawa Code Project Service initialized for project: " + project.getName());
    ...
    private void registerEventHandlers() {
        ...
        if (appService != null) {
            ...
            EventHandler authLoginHandler = new AuthLoginHandler(project);
            EventHandler authLogoutHandler = new AuthLogoutHandler(project);
            ...
            EventHandler branchSelectHandler = new BranchSelectHandler(project, diffViewerManager);
            EventHandler branchUnselectHandler = new BranchUnselectHandler(project, diffViewerManager);
            ...
            EventHandler syncRefreshHandler = new SyncRefreshHandler(project);

            appService.getEventDispatcher().registerHandler(authInfoHandler);
            appService.getEventDispatcher().registerHandler(authLoginHandler);
            appService.getEventDispatcher().registerHandler(authLogoutHandler);
            ...
            appService.getEventDispatcher().registerHandler(branchUnselectHandler);
            ...
            appService.getEventDispatcher().registerHandler(syncRefreshHandler);

            // Keep references to unregister later
            registeredHandlers.add(authInfoHandler);
            registeredHandlers.add(authLoginHandler);
            registeredHandlers.add(authLogoutHandler);
    ...
    /**
     * Send a sync:setup request to Gardener (fire-and-forget).
     * Triggers a cloud sync so highlights are up to date.
     */
    public void setupSync() {
        try {
            ...
            if (appService == null || !appService.isConnected()) {
                Logger.debug("Cannot setup sync: not connected");
                return;
            }

            Message message = MessageBuilder.buildSyncSetup(appService.getClientGuid());

            if (appService.getIpcConnection() != null) {
                appService.getIpcConnection().sendMessage(message);
                Logger.debug("Sent sync:setup request");
            }
        } catch (Exception e) {
            Logger.warn("Failed to send sync:setup request", e);
        }
    }
    ...
    @Override
    public void dispose() {
        Logger.info("Disposing Kawa Code Project Service for project: " + project.getName());
