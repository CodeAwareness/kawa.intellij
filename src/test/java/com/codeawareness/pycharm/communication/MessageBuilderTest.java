...
class MessageBuilderTest {
    ...
    @Test
    void testBuildHandshake() {
        Message message = MessageBuilder.buildHandshake();

        assertEquals(Message.Flow.REQ, message.getFlow());
        assertEquals("system", message.getDomain());
        assertEquals("handshake", message.getAction());
        assertNull(message.getCaw());

        // Data should contain clientType
        assertNotNull(message.getData());
        assertTrue(message.getData().isJsonObject(), "Data should be a JsonObject");
        JsonObject data = message.getData().getAsJsonObject();
        assertEquals("intellij", data.get("clientType").getAsString());
    }

    @Test
    void testBuildClientIdDelegatesToHandshake() {
        // buildClientId is deprecated and should delegate to buildHandshake
        Message message = MessageBuilder.buildClientId("any-guid");
    ...
    void testBuildAuthInfo() {
        ...
        assertEquals(Message.Flow.REQ, message.getFlow());
        assertEquals("auth", message.getDomain());
        assertEquals("info", message.getAction());
        assertEquals(guid, message.getCaw());
    }
